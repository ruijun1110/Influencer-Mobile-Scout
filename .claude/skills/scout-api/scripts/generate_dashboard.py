...
# ///
"""
generate_dashboard.py — Reads data/influencers.xlsx → writes data/dashboard.html.

Called at end of scout.py. Generates a self-contained HTML dashboard
with influencer cards and candidates table.
"""
import json
import sys
...
<title>TikTok Scout Dashboard</title>
<style>
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .card .view-stat .v-label {{
    font-size: 10px; color: var(--text3); text-transform: uppercase; letter-spacing: .3px;
  }}
  .card .view-stat .v-value {{
  ...
  .badge-auditing   {{ background: #e8f1fb; color: var(--accent); }}
...
<div class="content">
  ...
  <div id="influencers" class="section active">
    ...
    <div class="controls">
      <label>Sort by</label>
      <select class="filter-select" onchange="renderInfluencers()">
...
<footer>TikTok Scout &nbsp;·&nbsp; {generated_at}</footer>
...
function getSortKey()  {{ return document.querySelector('#influencers .controls select').value; }}
...
function renderInfluencers() {{
  ...
  if (!rows.length) {{
    grid.innerHTML = '<div class="empty" style="grid-column:1/-1"><div class="icon">🔍</div>No influencers match the current filters.</div>';
    return;
  }}
  grid.innerHTML = rows.map(r => `
    <div class="card">
      <div class="card-header">
        <div class="handle"><a href="${{r.profile_url || 'https://www.tiktok.com/@'+r.handle}}" target="_blank">@${{r.handle}}</a></div>
      </div>
      ...
      <div class="tags">
        ${{r.keyword  ? `<span class="tag tag-kw">#${{r.keyword}}</span>` : ''}}
        ${{r.campaign ? `<span class="tag tag-campaign">${{r.campaign}}</span>` : ''}}
      </div>
...
function renderCandidates() {{
  ...
  if (!rows.length) {{
    tbody.innerHTML = '<tr><td colspan="5" class="empty">No candidates match the current filters.</td></tr>';
    return;
  }}
  tbody.innerHTML = rows.map(r => {{
    ...
    return `<tr>
      ...
      <td>${{r.keyword ? `<span class="tag tag-kw" style="font-size:12px">#${{r.keyword}}</span>` : ''}}</td>
      <td>${{r.campaign || ''}}</td>
